...
- **Toast system is purely frontend**: The `showToast()` function added in Phase 5 is client-side only. No backend changes are required. Existing API success/error responses are presented as toasts instead of blocking `alert()` dialogs. All endpoints remain unchanged.
- **Phase 6 UI overhaul is purely frontend**: The complete visual redesign (Phase 6 in frontend.md) — new color tokens, indigo accent replacing amber, typography overhaul, timer ring resize, ghost borders — requires zero backend changes. All API endpoints, database schema, and server logic remain unchanged. The only file edited is `static/index.html`.
